...
| `CONCURRENCY`          | `8`                   | скільки номерів обробляється паралельно за запит |
...
| `CARGOAI_MIN_GAP_MS`   | `1080`                | пауза в межах однієї черги-ключа CargoAI (анти-429 per-sec) |
...
- **Ліміт тривалості функції на Vercel Hobby — 60 с.** У live-режимі номери
  обробляються паралельно (до `CONCURRENCY`, типово 8), тож пакет із ~10 номерів
