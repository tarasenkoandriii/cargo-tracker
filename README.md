...
| `SEA_TIMEOUT_MS`       | `15000`               | таймаут для морського джерела (Pier2Pier повільніше) |
...
| `CARGOAI_MIN_GAP_MS`   | `750`                 | пауза-черга між викликами CargoAI (анти-429 per-sec) |
| `RAPID_API_KEY_FALLBACK`| —                    | 2-й ключ RapidAPI (2-га квота): фоллбек при 429/квоті (аліас: `RAPIDAPI_KEY_FALLBACK`) |
...
1. **Pier2Pier** (`tracking2.php?Type=CONT&ID=…&Company=P2P`) — **безкоштовне**
   відстеження морських контейнерів. Віддає server-rendered HTML, тож працює
   на Vercel Hobby без браузера. Для контейнерів пробується **першим**.
2. **CargoAI API** (`GET /track?awb=…`) — для авіа (AWB), якщо задано
   `RAPIDAPI_KEY` або `CARGOAI_API_KEY`. Структуроване джерело, для авіа
   пробується першим.
3. **track-trace.com** — резерв (авіа `/aircargo`, море `/container`); на Hobby
   зазвичай недоступне (JS-рендеринг/анти-бот) → структурована помилка.
4. **Сайти перевізників** — резерв (шаблон-конектор для розширення).
...
- **Pier2Pier (море):** працює без браузера, але його `robots.txt` забороняє
  автоматичний доступ — користуйтеся помірно й на малих обсягах. З датацентрових
  IP (Vercel) сайт може блокувати/обмежувати; тоді конектор віддає
  структурований `SOURCE_UNAVAILABLE`. Розмітку розбираємо евристично (таблиця
  подій надійна; POL/POD/ETA/лінія — best-effort) — точка доводки в `parse()`.
...
- **Ліміт тривалості функції на Vercel Hobby — 60 с.** У live-режимі номери
  обробляються паралельно (до `CONCURRENCY`, типово 10), тож пакет із ~10 номерів
